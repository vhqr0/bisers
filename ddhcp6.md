...
以服务端在统计学上随机地生成地址，且探测次数n远小于地址池地大小为前提，
我们把如何评估该算法归化为以下数学问题：
...
过计算不难验证，当探测次数为200时，伪地址池的准确率超过99%。

我们可以继续改进该算法：假设得到的伪地址池为[a',b']，求未探测到的地址
池的大小的期望delta=2x(b'-a')/(n-1)，可以把[a'-delta,b'+delta]当做一个
更准确的伪地址池。
...
们尝试我们的算法在两种实现不同大小的子网：16位、24位、32位、40位中，先
请求1000个地址，再使用不同的探测次数：32、64、128、256探测地址池边界，
然后统计在该地址池中的地址占请求到的地址的比例，以此作为扫描的准确率。
测试结果如下：

TODO

虽然与基于Rebind的DHCPv6定界算法相比要发送更多的报文，且得到的地址并不
精确，但该算法仍是可以在几秒内探测到一个相对准确的地址池的更通用的优秀
的算法。
